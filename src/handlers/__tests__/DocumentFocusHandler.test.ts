...
describe('DocumentFocusHandler', () => {
  ...
  it('pollActiveElement returns focus to player when activeElement outside player', () => {
    ...
    mockPlayerManager.getPlayer.mockReturnValue(player as ReturnType<PlayerManager['getPlayer']>);
  ...
  it('pollActiveElement does nothing when activeElement equals lastActiveElement', () => {
    const pipDoc = document.implementation.createHTMLDocument();
    const player = pipDoc.createElement('div');
    const outer = pipDoc.createElement('div');
    pipDoc.body.appendChild(player);
  ...
  it('pollActiveElement does nothing when activeElement is player', () => {
    const pipDoc = document.implementation.createHTMLDocument();
    const player = pipDoc.createElement('div');
    pipDoc.body.appendChild(player);
    const focusFn = vi.fn();
  ...
  it('pollActiveElement does nothing when activeElement is inside player', () => {
    const pipDoc = document.implementation.createHTMLDocument();
    const player = pipDoc.createElement('div');
  ...
  it('pollActiveElement does nothing when context menu open', () => {
    const pipDoc = document.implementation.createHTMLDocument();
    const player = pipDoc.createElement('div');
    const outer = pipDoc.createElement('div');
    pipDoc.body.appendChild(player);
  ...
  it('pollActiveElement does nothing when activeElement is null', () => {
    const pipDoc = document.implementation.createHTMLDocument();
    const player = pipDoc.createElement('div');
    pipDoc.body.appendChild(player);
    player.focus = vi.fn();
  ...
  it('pollActiveElement does nothing when player.focus is not a function', () => {
    const pipDoc = document.implementation.createHTMLDocument();
    const player = pipDoc.createElement('div');
    const outer = pipDoc.createElement('div');
    pipDoc.body.appendChild(player);
    pipDoc.body.appendChild(outer);
    delete (player as unknown as { focus?: unknown }).focus;
    mockPlayerManager.getPlayer.mockReturnValue(player as ReturnType<PlayerManager['getPlayer']>);
    let pollCb: () => void = () => {};
    vi.stubGlobal('setInterval', (cb: () => void) => {
      pollCb = cb;
  ...
  it('subscribe callback with false calls pollActiveElement', () => {
    const pipDoc = document.implementation.createHTMLDocument();
    const player = pipDoc.createElement('div');
    player.focus = vi.fn();
    mockPlayerManager.getPlayer.mockReturnValue(player as ReturnType<PlayerManager['getPlayer']>);
