...
| **VPC**                                                          | **Public** subnet: internet gateway + **NAT instance** + **Elastic IP**. **Private** subnet: Fargate tasks; default route **`0.0.0.0/0`** targets the **NAT instance’s primary ENI** (not a managed NAT Gateway). Includes interface VPC endpoints for **ECR API** and **ECR DKR**. |
...
5. **Fargate** pulls the runner image from **ECR** via private VPC interface endpoints (`ecr.api`, `ecr.dkr`), reducing NAT usage for image pull traffic.
